...
export default function ProductPage({ product_data, merchant_data }: Param) {
    ...
    const [isValidBuyer, setIsOwner] = useState(false);
    useEffect(() => {
    ...
    console.log("isValidBuyer: ", isValidBuyer)
            ...
            {bought ? (
                <div className="fullW fullH" style={{position: "fixed", top: 0, left: 0, background: "rgba(0, 0, 0, 0.75)", zIndex: 4, color: "white"}}>
                    <div className='flex fullW fullH verti centerHori centerVerti'>
                        <span className='em1_5 b' style={{color: "white", textAlign: "center"}}> You have bought this item. </span>
                        <span className='em1_5 b' style={{color: "white", textAlign: "center"}}> You can access your product using <a style={{color: 'aqua', textDecoration: "underline"}} href={'http://localhost:8000/storage/' + product_data.product_location}> this link. </a> </span>
                        <br />
                        <br />
